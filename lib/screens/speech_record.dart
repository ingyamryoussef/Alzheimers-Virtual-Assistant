...
class speech_record extends StatefulWidget {
  const speech_record({Key? key}) : super(key: key);
  @override
  State<speech_record> createState() => _speech_recordState();
}

class _speech_recordState extends State<speech_record> {
  ...
  Widget build(BuildContext context) {
    ...
    int myspeakerid=samplerecordmap["speaker_id"];

    return Scaffold(
      appBar: AppBar(
        title: Text('Record Conversation', style: TextStyle(fontSize:25),),
        centerTitle: true,
        toolbarHeight: 100.0,
          ...
          children: <Widget>[
            ...
            ElevatedButton.icon(
              ...
              onPressed:() async{
                ...
                recorder.speaker_ID= await samplerecordmap["speaker_id"];
                recorder._uploadVoiceFile();
                Navigator.pushReplacementNamed(context, '/home', arguments: '''{"name":"${samplerecordmap["name"]}","email":"${samplerecordmap["email"]}","id":"${samplerecordmap["id"]}"}''');
                setState(() {});
              },
            )
          ],
...
class SoundRecorder {
  ...
  Future<void> _uploadVoiceFile() async {
    ...
    if(filePath!=""){
      final url = Uri.parse('http://${APIConfig.ipAddress}/conversation_analysis/conv_audio_upload/');
      final request = http.MultipartRequest('POST', url);
      request.fields["email"]=email;
      request.fields["speaker_id"]=speaker_ID.toString();
