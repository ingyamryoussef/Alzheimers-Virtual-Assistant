...
class _signup_patientState extends State<signup_patient> {
  ...
  final controllerCaregiverEmail=TextEditingController();
  String alert = '';
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          SizedBox(height: 30.0,),
                          TextFormField(
                            decoration: SignupInputDecoration('Caregiver Email*'),
                            style: TextStyle(color: Colors.white),
                            controller: controllerCaregiverEmail,
                          ),
                          ...
                          a.GradientFloatingActionButton.extended(
                            onPressed: () {
                              ...
                              final caregiver_email=controllerCaregiverEmail.text;
                              Upload(email, password, first_name, last_name, caregiver_email);
                            },
                            label: Text(
                              "Signup",
                              style: TextStyle(
  ...
  void signupFunction() async{
    ...
    if(controllerFirstName.text!='' && controllerLastName.text!='' && controllerEmail.text!='' && controllerPassword.text!='' && controllerConfirmPassword.text!='' && controllerEmail.text.contains('@') && controllerPassword.text.length>=6 && controllerConfirmPassword.text==controllerPassword.text && backendValid ) {
      alert = 'success';
      await Navigator.pushReplacementNamed(context, '/tutorial', arguments:'''{"name":"$userfirstname" , "email":"$useremail"}''');
      setState(() {
  ...
  Future Upload(email,password,first_name,last_name,caregiver_email) async{
    // var request = http.MultipartRequest('POST',Uri.http('192.168.1.5:8001' , 'caregiverpatient/signup/' ));
    ...
    // Check the response status code
      ...
      else if(controllerCaregiverEmail.text=='' || !controllerCaregiverEmail.text.contains('@'))
      {
        setState(() {
          alert='Enter a valid caregiver Email';
        });
      }
      else{
        var request = http.MultipartRequest('POST',Uri.http(APIConfig.ipAddress, 'speaker_recognition/patients/signup/' ));
        request.fields['email']=email;
        request.fields['password']=password;
        request.fields['first_name']=first_name;
        request.fields['last_name']=last_name;
        request.fields['caregiver_email']=caregiver_email;
